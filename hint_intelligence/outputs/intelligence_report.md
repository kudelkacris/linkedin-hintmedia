# HINT INTELLIGENCE ENGINE — Reporte
**Generado:** 2026-07-01  |  **Total contactos:** 427  |  **Versión KB:** 10

---

## EMBUDO GLOBAL

| Etapa | N | % |
|---|---|---|
| Respondió MSG1 | 23 | 5.4% |
| Dossier enviado | 107 | 25.1% |
| Seguimiento SEG | 55 | 12.9% |
| Avanzado/Reunión | 10 | 2.3% |

---

## SECTORES — Conversión a dossier

| Sector | N | Resp% | Dossier% | Seg% | Confianza |
|---|---|---|---|---|---|
| Real Estate | 8 | 0.0% | **37.5%** | 37.5% | ⚫ INSUFFICIENT |
| Educación | 42 | 2.4% | **35.7%** | 7.1% | 🟡 MEDIUM |
| Minería | 14 | 0.0% | **35.7%** | 14.3% | 🔴 LOW |
| Salud/Clínica | 6 | 0.0% | **33.3%** | 16.7% | ⚫ INSUFFICIENT |
| Seguros/Finanzas | 9 | 11.1% | **33.3%** | 22.2% | ⚫ INSUFFICIENT |
| Consultoría/Legal | 10 | 10.0% | **30.0%** | 10.0% | 🔴 LOW |
| Agencia/Marketing | 25 | 8.0% | **28.0%** | 12.0% | 🔴 LOW |
| Energía | 26 | 7.7% | **26.9%** | 7.7% | 🔴 LOW |
| Agro/Alimentos | 4 | 0.0% | **25.0%** | 0.0% | ⚫ INSUFFICIENT |
| Farmacéutico | 20 | 10.0% | **25.0%** | 5.0% | 🔴 LOW |
| Tecnología | 20 | 10.0% | **25.0%** | 5.0% | 🔴 LOW |
| Turismo/Hotelería | 14 | 7.1% | **14.3%** | 7.1% | 🔴 LOW |
| Retail/Consumo | 16 | 0.0% | **12.5%** | 12.5% | 🔴 LOW |
| Automotriz | 3 | 0.0% | **0.0%** | 0.0% | ⚫ INSUFFICIENT |
| Construcción/Materiales | 3 | 0.0% | **0.0%** | 0.0% | ⚫ INSUFFICIENT |
| Logística/Transporte | 4 | 0.0% | **0.0%** | 0.0% | ⚫ INSUFFICIENT |
| Medios/Comunicación | 3 | 0.0% | **0.0%** | 0.0% | ⚫ INSUFFICIENT |

---

## VARIANTES MSG1

| Variante | N | Resp% | Dossier% | Confianza |
|---|---|---|---|---|
| Variante A | 260 | 5.8% | **28.5%** | 🟢 HIGH |
| Variante C | 67 | 6.0% | **19.4%** | 🟡 MEDIUM |

---

## SENIORITY

| Seniority | N | Resp% | Dossier% | Confianza |
|---|---|---|---|---|
| DIRECTOR | 11 | 0.0% | **27.3%** | 🔴 LOW |
| OTHER | 363 | 6.1% | **25.9%** | 🟢 HIGH |
| SPECIALIST | 8 | 0.0% | **25.0%** | ⚫ INSUFFICIENT |
| MANAGER | 34 | 2.9% | **20.6%** | 🟡 MEDIUM |
| CEO | 10 | 0.0% | **10.0%** | 🔴 LOW |

---

## COMBOS (Sector × Seniority, mín. 5 casos)

| Combo | N | Dossier% | Confianza |
|---|---|---|---|
| Educación × OTHER | 23 | **52.2%** | 🔴 LOW |
| Seguros/Finanzas × OTHER | 7 | **42.9%** | ⚫ INSUFFICIENT |
| Minería × OTHER | 10 | **40.0%** | 🔴 LOW |
| Agencia/Marketing × OTHER | 22 | **31.8%** | 🔴 LOW |
| Real Estate × OTHER | 7 | **28.6%** | ⚫ INSUFFICIENT |
| Energía × OTHER | 20 | **25.0%** | 🔴 LOW |
| Tecnología × OTHER | 17 | **23.5%** | 🔴 LOW |
| Farmacéutico × OTHER | 18 | **22.2%** | 🔴 LOW |
| Consultoría/Legal × OTHER | 5 | **20.0%** | ⚫ INSUFFICIENT |
| Energía × MANAGER | 5 | **20.0%** | ⚫ INSUFFICIENT |
| Retail/Consumo × OTHER | 13 | **15.4%** | 🔴 LOW |
| Educación × MANAGER | 11 | **9.1%** | 🔴 LOW |
| Turismo/Hotelería × OTHER | 12 | **8.3%** | 🔴 LOW |
| Educación × DIRECTOR | 5 | **0.0%** | ⚫ INSUFFICIENT |

---

## ENGAGEMENT LEVEL (desde conversaciones/.md)

| Engagement | N | Dossier% | Reunión% | Confianza |
|---|---|---|---|---|
| HIGH | 13 | **69.2%** | 15.4% | 🔴 LOW |
| MEDIUM | 51 | **66.7%** | 2.0% | 🟡 MEDIUM |
| LOW | 56 | **30.4%** | 5.4% | 🟡 MEDIUM |

---

## OBJECIONES MÁS FRECUENTES

| Objeción | N | Confianza |
|---|---|---|
| PARTNERSHIP | 10 | 🔴 LOW |
| BAD_TIMING | 5 | ⚫ INSUFFICIENT |
| CURIOSITY_ONLY | 5 | ⚫ INSUFFICIENT |
| HAS_AGENCY | 4 | ⚫ INSUFFICIENT |

---

## INSIGHTS ACCIONABLES

### 📊 Mejor sector: Real Estate (37.5% dossier, n=8)
- **Confianza:** ⚫ INSUFFICIENT
- **Acción:** Priorizar contactos en Real Estate

### 📊 Variante A supera a C: 28.5% vs 19.4% dossier
- **Confianza:** 🟡 MEDIUM
- **Acción:** Default Variante A. Aumentar casos de Variante C para confirmar (n=67, necesita ≥30 para confianza MEDIUM)

### 🎯 CEO convierte menos en frío: 10.0% vs Director 27.3%
- **Confianza:** 🔴 LOW
- **Acción:** Preferir Director/Manager como primer contacto. Escalar a CEO solo si el Director lo sugiere.

### 🧪 Consultoría/Legal: 30.0% dossier pero n=10 (insuficiente para confirmar)
- **Confianza:** 🔴 LOW
- **Acción:** Aumentar volumen en Consultoría/Legal para confirmar el patrón. Objetivo: n=30.

### 🧪 Minería: 35.7% dossier pero n=14 (insuficiente para confirmar)
- **Confianza:** 🔴 LOW
- **Acción:** Aumentar volumen en Minería para confirmar el patrón. Objetivo: n=30.

### 🧪 Real Estate: 37.5% dossier pero n=8 (insuficiente para confirmar)
- **Confianza:** 🔴 LOW
- **Acción:** Aumentar volumen en Real Estate para confirmar el patrón. Objetivo: n=30.

### 🧪 Salud/Clínica: 33.3% dossier pero n=6 (insuficiente para confirmar)
- **Confianza:** 🔴 LOW
- **Acción:** Aumentar volumen en Salud/Clínica para confirmar el patrón. Objetivo: n=30.

### 🧪 Seguros/Finanzas: 33.3% dossier pero n=9 (insuficiente para confirmar)
- **Confianza:** 🔴 LOW
- **Acción:** Aumentar volumen en Seguros/Finanzas para confirmar el patrón. Objetivo: n=30.


---

## EXPERIMENTOS PROPUESTOS

### [~] EXP_SECTOR_CONSULTORÍA_LEG
**Hipótesis:** Consultoría/Legal convierte al 30.0% — confirmar con más volumen
**Por qué:** Solo 10 casos. Con n<30 no es concluyente.
**Cómo testear:** Buscar y contactar 20 prospectos más en Consultoría/Legal.
**Progreso:** 10/30 casos · Faltan: 20
**Impacto si confirma:** Priorizar Consultoría/Legal como sector de alto rendimiento.

### [~] EXP_SECTOR_MINERÍA
**Hipótesis:** Minería convierte al 35.7% — confirmar con más volumen
**Por qué:** Solo 14 casos. Con n<30 no es concluyente.
**Cómo testear:** Buscar y contactar 16 prospectos más en Minería.
**Progreso:** 14/30 casos · Faltan: 16
**Impacto si confirma:** Priorizar Minería como sector de alto rendimiento.

### [~] EXP_CEO_APPROACH
**Hipótesis:** CEO convierte mejor si primero se conecta con alguien de su equipo (Director/Manager)
**Por qué:** CEO directo = 10.0% dossier (n=10). Posible que el approach indirecto sea más efectivo.
**Cómo testear:** Próximos 10 CEOs: no contactar al CEO directamente. Primero conectar con Director/Manager del área. Trackear si la tasa de dossier sube.
**Progreso:** 0/10 casos · Faltan: 10
**Impacto si confirma:** Cambiar la estrategia de targeting para empresas con CEO conocido.

### [·] EXP_SECTOR_REAL_ESTATE
**Hipótesis:** Real Estate convierte al 37.5% — confirmar con más volumen
**Por qué:** Solo 8 casos. Con n<30 no es concluyente.
**Cómo testear:** Buscar y contactar 22 prospectos más en Real Estate.
**Progreso:** 8/30 casos · Faltan: 22
**Impacto si confirma:** Priorizar Real Estate como sector de alto rendimiento.

### [·] EXP_SECTOR_SALUD_CLÍNICA
**Hipótesis:** Salud/Clínica convierte al 33.3% — confirmar con más volumen
**Por qué:** Solo 6 casos. Con n<30 no es concluyente.
**Cómo testear:** Buscar y contactar 24 prospectos más en Salud/Clínica.
**Progreso:** 6/30 casos · Faltan: 24
**Impacto si confirma:** Priorizar Salud/Clínica como sector de alto rendimiento.

### [·] EXP_SECTOR_SEGUROS_FINANZA
**Hipótesis:** Seguros/Finanzas convierte al 33.3% — confirmar con más volumen
**Por qué:** Solo 9 casos. Con n<30 no es concluyente.
**Cómo testear:** Buscar y contactar 21 prospectos más en Seguros/Finanzas.
**Progreso:** 9/30 casos · Faltan: 21
**Impacto si confirma:** Priorizar Seguros/Finanzas como sector de alto rendimiento.

---

## LEYENDA DE CONFIANZA

| Nivel | N casos | Significado |
|---|---|---|
| 🟢 HIGH | ≥ 100 | Regla establecida |
| 🟡 MEDIUM | 30–99 | Patrón consistente |
| 🔴 LOW | 10–29 | Hipótesis a confirmar |
| ⚫ INSUFFICIENT | < 10 | Sin datos suficientes |

_Generado por Hint Intelligence Engine — Fase 1: Observación_